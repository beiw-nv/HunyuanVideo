...
ATTN=torch_cudnn
PROFILE_FILE=hunyuanvideo_inference_${ATTN}_b200_trt
...
if [ $MULTI_GPU -eq 0 ]; then
	...
	#nsys profile -w true -t cuda,nvtx,osrt,cudnn,cublas --capture-range=cudaProfilerApi --capture-range-end=stop --cudabacktrace=true -x true -f true -o $PROFILE_PATH/${PROFILE_FILE}_fp8 \
	    python3 sample_video.py --dit-modelopt-weight ${DIT_MODELOPT_CKPT_PATH} --video-size 720 1280 --video-length 129 --infer-steps 3 --prompt "A cat walks on the grass, realistic style." --flow-reverse --flow-shift 7.0 --embedded-cfg-scale 6.0 --seed 42 --save-path ./results --vae-trt --use-modelopt-fp8 --model-trt #--use-modelopt-fp8
